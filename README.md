...
```bash
git clone https://github.com/jychpr/SAM-PL-OVD.git
cd SAM-PL-OVD/
```
2. Create environment
```bash
conda create -n SPO python=3.10 -y
conda activate SPO
```
...
This repository enforces a strict "Offline Geometry" pipeline to prevent GPU bottlenecks:

Pre-computation: Run the scripts in tools/sam_priors/ to generate .pt tensor files containing the bounding box priors for the target dataset.

Training/Evaluation: Execute the standard main.py distributed loops. The modified datasets/ov_coco.py and models/ov_dquo.py will dynamically load the pre-computed tensors and inject them into the modified Deformable Transformer.
...
- data/Anotations/instances_val2017_basetarget_BACKUP.json
